...
"[project]/src/app/api/scrape/stream/[id]/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(req, { params }) {
    const { id } = params;
    if (!id) {
        return new Response("Missing job_id parameter", {
            status: 400
        });
    }
    // Call FastAPI backend SSE stream
    const backendResponse = await fetch(`${process.env.BACKEND_API}scrape/stream/${id}`);
    // Check if response is valid
    if (!backendResponse.ok || !backendResponse.body) {
        return new Response("Failed to connect to backend SSE", {
            status: 500
        });
    }
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__3032f6._.js.map